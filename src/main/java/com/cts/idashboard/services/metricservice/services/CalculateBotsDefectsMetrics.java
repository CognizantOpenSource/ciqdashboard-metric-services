...
public class CalculateBotsDefectsMetrics {
    ...
    public MetricResults calculateBotsMetrics(String metricName, ProjectMetric projectMetric,String SourceBotsDefects) throws Exception {
        ...
        if(projectMetric.getCustomFunction().equalsIgnoreCase("Yes")) {
            ...
            projectMetric = customFunctions.executeCustomFunction(projectMetric, "SourceBotsDefects");
        }
            ...
            for (Map.Entry<String, JSONObject> map : formulaParams.entrySet()) {
                ...
                for (Map.Entry<String, JSONObject> valObj : valueObjectMap.entrySet()) {
                    ...
                    if (valObj.getKey().equals("count")) {
                        ...
                        Double valKey = metricFunctions.sum("SourceBotsDefects", valueMap.entrySet(), projectMetric, "No", isGrouping, null, null, distinctValue);
                        variableSet.set(map.getKey(), valKey);
                    } else if (valObj.getKey().equals("sumAnd")) {
                        Double valKey = metricFunctions.sumAnd("SourceBotsDefects", valueMap.entrySet(), projectMetric, "No", isGrouping, null, null, distinctValue);
                        variableSet.set(map.getKey(), valKey);
